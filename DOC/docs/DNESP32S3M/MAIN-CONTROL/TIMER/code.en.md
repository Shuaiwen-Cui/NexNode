...
!!! note
    ...
    Also, we use the LED to show the effect of the external interrupt, so we need to add `led` in the `REQUIRES` field. Apart from that, for timer, we need to add `esp_timer` in the `REQUIRES` field.
