...
!!! note
    注意，在驱动程序中，我们使用了 ESP-IDF 内置的 `driver` 库中的spi和gpio相关函数，因此，我们需要在 `CMakeLists.txt` 文件的 `REQUIRES` 字段中指明这个依赖关系。
...
!!! note "注意"
    以上是在 ESP32-S3 上设置 SPI2 的代码。如果使用其他SPI外设，则需要相应配置相应的引脚和SPI主机。例如，我们以后的SD卡将使用SPI3，则需要相应配置引脚和SPI主机。