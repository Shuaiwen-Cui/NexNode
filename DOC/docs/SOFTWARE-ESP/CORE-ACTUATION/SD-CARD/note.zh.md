...
本章实验功能简介：经过一系列初始化之后，通过一个 while 循环以 SD 卡初始化为条件，以检测 SD 卡是否初始化成功，若初始化 SD 卡成功，则会通过串口或者 VSCode 终端输出 SD卡的相关参数。此时 LED 闪烁，表示程序正在运行。
...
本章实验使用 SPI 接口与 SD 卡进行连接，节点板载了一个 Micro SD卡座用于连接 SD 卡，SD 卡与 ESP32-S3 的连接原理图，如下图所示。

![TF](TF.png)

## 依赖关系

本部分依赖的组件有sdmmc,vfs,fatfs,driver,node_spi等。