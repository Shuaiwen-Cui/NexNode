...
激活 ESP-IDF 环境。然后输入以下命令创建一个名为 `node_acc_adxl355` 的新组件：

```bash
idf.py -C driver create-component node_acc_adxl355
```

上面的命令意味着在 `driver` 目录中创建一个名为 `node_acc_adxl355` 的新组件。执行命令后，您将在 `driver` 目录中看到一个名为 `node_acc_adxl355` 的新目录。命令将自动生成新组件的 `CMakeLists.txt` 文件，以及 `node_acc_adxl355.h` 和 `node_acc_adxl355.c` 文件。
...
将本节中代码分别替换到组件下的`CMakelists.txt`、`node_acc_adxl355.h`和`node_acc_adxl355.c`文件中。
