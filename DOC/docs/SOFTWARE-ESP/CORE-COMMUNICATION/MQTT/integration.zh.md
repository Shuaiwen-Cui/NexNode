...
!!! danger
    注意，为了避免冲突，我们将这个模块命名为node_mqtt。以前ESPRESSIF是内置mqtt模块的，但后来ESPRESSIF将mqtt模块从内置模块中剥离出来了，因此现在ESP-IDF里没有内置mqtt模块了。我们的node_mqtt模块还是依赖这个库，但是这个库已经不属于ESP-IDF的内置模块了，现在要想使用要么使用在线下载的方式，要么使用离线包的方式。我们这里使用离线包的方式，我已经把这个库放在driver目录下了，请确保这个包也在你的项目driver目录下。
