...
1. `node_espnow_init failed`  
   - 检查 Wi-Fi/ESP-NOW 初始化时序与依赖组件。
2. `node_espnow_send_to` 返回 `ESP_ERR_INVALID_STATE`  
   - 前一会话尚未结束；等待回调或降低发送节奏。
3. 大包频繁超时  
   - 减小 `chunk_payload_bytes`，增大 `ack_timeout_ms`，窗口先保持 1。
4. 发现阶段成功但业务阶段失败  
